...
###4.2.1 GGplot----
ggplot(lstmeg) +
  geom_smooth(aes(x = day, y = Temp, color = "Temperatura_ERA5")) +
  geom_smooth(aes(x = day, y = TempCday, color = "LST_MODIS_day")) +
  geom_smooth(aes(x = day, y = TempCnight, color = "LST_MODIS_night")) +
  geom_smooth(aes(x = day, y = TempC, color="LST_ECOSTRESS"))+
  facet_wrap(~Category)+
  labs(
    title = "Temperatura lak GRASSAT w Wielkopolsce",
    x = "Data pomiaru",
    y = "Temperatura (°C)",
    caption = "Na podstawie danych NASA ECOSTRESS, MODIS, ERA5"
  )+
  theme_minimal()
...
ECO_2020 <- ET_ECOSTRESS1[format(ET_ECOSTRESS1$Date, "%Y") == "2020",]

ECO_2020 %>% glimpse()

## Filtring data - months from 03 to 10

filtered_data <- ECO_2020 %>%
  mutate(Month = as.numeric(format(Date, "%m"))) %>%
...
ggplot(filtered_data) +
  ...
  facet_wrap(~Category)+
  labs(
    title = "Ewapotranspiracja (ET= Ts- ta) na lakach GRASSAT w Wielkopolsce w 2020 roku",
    x = "Data",
    y = "Ewapotranspiracja (Ts- Ta)",
    caption = "Na podstawie danych NASA ECOSTRESS"
  )+
